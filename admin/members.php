			...
			if($_SERVER['REQUEST_METHOD'] === 'POST'){
				...
				if(empty($formErrors)){
					...
					$addUser = insertItems('users',
									'username, password, Email, FullName, `Date`, admin',
									'?, ?, ?, ?, now(), ?',
									array($username, $hashedPass, $email, $fullname, $admin));
				...
				if(empty($formErrors)){
					...
					if(empty($password)){ //if the password input is empty then the user didn't change the password
						$updateUser = updateItems('users', 
										...
										array($username, $email, $fullname, $admin, $userid), 
										'id = ?');
					}else{ //if the password field not empty then the user changed the password
						$updateUser = updateItems('users', 
										'username = ?, password = ?, Email = ?, FullName = ?, admin = ?', 
										array($username, $hashedPass, $email, $fullname, $admin, $userid), 
										'id = ?');
					}
